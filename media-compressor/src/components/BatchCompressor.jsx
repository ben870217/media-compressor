...
import { Conversion, Input, Output, BlobSource, BufferTarget, MP4, MATROSKA, WEBM, Mp4OutputFormat } from 'mediabunny';
...
async function videoMeta(file) {
  ...
  try {
    const data = await new Promise((resolve, reject) => {
      ...
      video.onerror = () => reject(new Error('無法讀取影片 metadata 或格式不受支援。'));
    });
    return data;
...
export default function BatchCompressor({ type, onCompressComplete }) {
  ...
  const processVideo = async (item, settings, dimensions, attempt) => {
    const input = new Input({ source: new BlobSource(item.file), formats: [MP4, MATROSKA, WEBM] });
  ...
  const runQueue = async (retryOnly = false) => {
    ...
    for (const source of snapshot) {
      ...
      try {
        ...
        updateItem(item.id, { status: 'success', progress: 100, result: { blob: result, url: resultUrl, dimensions, overTarget, attempts } });
  ...
  const download = (item) => { const link = document.createElement('a'); link.href = item.result.url; link.download = `${sanitizeFilename(item.file.name.replace(/\.[^.]+$/, ''))}_compressed.${type === 'video' ? 'mp4' : item.result.blob.type === 'image/webp' ? 'webp' : 'jpg'}`; link.click(); setTimeout(() => URL.revokeObjectURL(item.result.url), 2000); };
  ...
  return <div className="batch-workspace">
    ...
    <section className="batch-settings"><h3>批次共用設定</h3><Settings settings={activeSettings} type={type} onChange={updateBase} /></section>
    {items.length > 0 && <>
      ...
      <div className="batch-queue">{items.map((item, index) => <article className={`queue-item ${item.status}`} key={item.id} draggable={!running} onDragStart={() => setDraggedId(item.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedId) moveItem(draggedId, item.id); setDraggedId(null); }}>
        ...
        {item.result && <div className="result"><span>{item.result.dimensions.width} × {item.result.dimensions.height} · {(item.result.blob.size / 1024 ** 2).toFixed(2)} MB</span>{item.result.overTarget && <span className="warn">已重試 2 次，保留最接近目標的結果。</span>}<button onClick={() => download(item)}>下載 / 重新命名</button></div>}
...
function Settings({ settings, type, onChange, overridden = {}, onReset }) {
  ...
  return <div className="settings-grid">
    {field('targetSize', <>目標容量（MB）<input type="number" min="0.1" step="0.1" value={settings.targetSize} onChange={(e) => onChange('targetSize', Math.max(.1, Number(e.target.value)))} /></>)}
    {field('longEdge', <>長邊上限（px）<select value={settings.longEdge} onChange={(e) => onChange('longEdge', Number(e.target.value))}><option value="3840">3840</option><option value="2048">2048</option><option value="1080">1080</option><option value="720">720</option><option value="480">480</option></select></>)}
