...
4. **音訊軌道優化 (64kbps Voice Audio & Silence Detection)**：預設音訊碼率調降至 64 kbps (語音最佳化)，靜音軌道自動跳出「建議移除無聲音訊軌」提示；UI 清晰對比來源與調整後音訊碼率。
...
4. As a user recording software demos without audio, I want the system to detect silent audio tracks and suggest stripping the audio stream, so that I can save extra megabytes with a single click.
...
- **音訊軌道優化 (`audio`)**:
  ...
  - 於 `videoMeta` 偵測靜音軌道，若為全靜音則跳出標籤提示移除音訊。
